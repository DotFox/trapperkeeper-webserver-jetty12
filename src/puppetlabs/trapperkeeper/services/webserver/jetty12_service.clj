(ns puppetlabs.trapperkeeper.services.webserver.jetty12-service
  (:require
    ...
    [puppetlabs.trapperkeeper.services.webserver.jetty12-core :as core]))
...
(defservice jetty12-service
  "Provides a Jetty 12 web server as a service"
  WebserverService
  ...
  (start [this context]
         ...
         (let [config-service (get-service this :ConfigService)
           ...
           (doseq [started-server-context (:jetty12-servers started-context)]
           ...
           (if-let [filesystem-watcher-service
             ...
             (let [watcher (watch-protocol/create-watcher filesystem-watcher-service {:recursive false})]
               (doseq [server (:jetty12-servers started-context)]
  ...
  (stop [this context]
        ...
        (doseq [key (keys (:jetty12-servers context))]
          (if-let [server (key (:jetty12-servers context))]
